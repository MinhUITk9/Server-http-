...
{
	m_httpHeaders.push_back(make_pair(headerName, headerContent)); //add element add the end of vector m_httpHeaders
	return 0;
}
...
int HTTPResponse::copyFromFile(ifstream& ifs, int contentLength)
{
	...
	if(ifs.good()){
		ifs.read(fileBuf, contentLength); // read data as a block:
	}
	m_responseBody.append(fileBuf, contentLength); //Appends a copy of the first number of "contentLength" characters 
													//in the array of characters pointed by "filebuf".
	if(ifs.bad())
		return -1;
